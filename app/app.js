...
const changeCase = (text) => {
  text.data = text.data.toUpperCase();
  console.log('file changed to upper case', text.data);
  updatingFile(text);
};
 ...
 * @param text
 */
const updatingFile = async (text) => {
  await fs.writeFile( text.file, Buffer.from(text.data), (err, data) => {
    if(err) {server.emit('file-error')}
